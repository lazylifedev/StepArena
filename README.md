...
StepArenaは、端末の歩数センサーで日々の活動を記録し、端末内で自動生成されたチャレンジパートナーとのデイリーチャレンジを楽しめる歩数計アプリです。

アカウント、ログイン、クラウド同期、実在ユーザーとのオンライン対戦、広告はありません。
...
- デイリーチャレンジ、歩行ランク、週間グループ、月間記録、達成記録
- 端末内データの使用状況、ZIPエクスポート、チャレンジ記録の初期化、全削除
- 計測停止診断とバッテリー設定案内
- TalkBack、動的文字サイズ、Reduce Motionを考慮したCompose UI

チャレンジパートナーは端末内で自動生成されます。チャレンジ対象歩数は1日30,000歩までですが、通常の歩数履歴は上限後も記録されます。本アプリは医療機器ではありません。
